...
class L10nHuBaseResCompany(models.Model):
    ...
    l10n_hu_api_license_valid = fields.Boolean(
        copy=False,
        readonly=True,
        string="HU API License Valid",
    )
    l10n_hu_api_registered = fields.Boolean(
        copy=False,
        readonly=True,
        string="HU API Registered",
    )
    l10n_hu_api_timestamp = fields.Datetime(
        copy=False,
    ...
    # Action methods
    def action_l10n_hu_api_check_registration(self):
        """ Open the wizard for check API registration """
        # Ensure one
        self.ensure_one()

        # Check
        if not self.l10n_hu_api_enabled:
    ...
    def action_l10n_hu_api_create_registration(self):
    ...
    def action_l10n_hu_api_delete_registration(self):
    ...
    def action_l10n_hu_api_get_objects(self):
    ...
    def action_l10n_hu_api_list_logs(self):
        """ List logs """
        # Ensure one
        self.ensure_one()
        ...
        # Set l10n_hu_log_ids
        l10n_hu_log_ids = []
        logs = self.env['l10n.hu.log'].sudo().search([
            ('company', '=', self.id),
    ...
    def action_l10n_hu_api_reset_registration(self):
        """ Reset registration to default """
        # Ensure one
        self.ensure_one()

        # Reset
        current_api_data = self.l10n_hu_get_api_data()
        api_data = {
            'api_key': 'free',
    ...
    # # API
    @api.model
    def l10n_hu_api_registration_request(self, values):
        ...
        :return: dictionary
        """
        # raise exceptions.UserError("l10n_hu_api_registration_request BEGIN" + str(values))
